...
class FindViableNodesService
{
    ...
    public function handle(int $perPage = null, int $page = null)
    {
        ...
        $query = Node::query()->select('nodes.*')
            ...
            ->selectRaw('SUM(s.memory) / (n.memory * (1 + (n.memory_overallocate / 100))) * 100 AS pct')
        ...
        $results = $query->groupBy('nodes.id')
            ...
            ->havingRaw('(((SUM(s.memory) + ?) / (n.memory * (1 + (n.memory_overallocate / 100)))) * 100) <= 60', [$this->memory])
            ->orderByRaw('n.memory', 'DESC')
